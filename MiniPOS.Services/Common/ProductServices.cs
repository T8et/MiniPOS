...
namespace MiniPOS.Services.Common
{
    public class ProductServices
    {
        ...
        public async Task<string> Create(BtProduct product) 
        {
            ...
            {
                var response = db.BtProductCats.AsNoTracking().Where(x => x.CatProductCode == catcode).FirstOrDefaultAsync();
                if (response != null)
                {
                    try
                    {
        ...
        public async Task<string> Update(string code, string desc, double price, string catcode)
        {
            ...
            if(response is not null)
            {
                ...
                if (price <= 0) response.ProductPrice = (decimal)price;
                if(catcode is not null)
                {
        ...
        {
            var response = db.BtProducts.AsNoTracking().Where(x => x.ProductCode == code).FirstOrDefaultAsync();
            if(response is not null)
            {
